  ...
  template: `
    <main class="mc-ai" *ngIf="isSuperAdmin; else aiAccessDenied">
      <header class="mc-topbar">
        <div>
          <p class="eyebrow">MADCloud intelligence service</p>
          <h1>/ai</h1>
          <p class="sub">{{ productName }} code fixes, assisted work, callbacks, and artifacts are routed through MADCloud Local.</p>
        </div>
        <div class="top-actions">
          ...
          <button type="button" class="primary" (click)="openNew()">New MADCloud task</button>
      ...
      <section class="task-list">
        <article class="empty" *ngIf="filteredTasks.length === 0">No MADCloud tasks match this view.</article>
...
export class MadcloudAiPage implements OnInit, OnDestroy {
  ...
  readonly sourceApp = 'madlearn';
  readonly productName = 'MADLearn';
