...
@Component({
  ...
  template: `
    <section class="payfast-subscribe" [class.payfast-subscribe--compact]="compact" aria-label="Payfast.io subscription">
      <div class="payfast-subscribe__copy">
        <p class="payfast-subscribe__eyebrow">Payfast.io secure checkout</p>
        ...
        <div class="payfast-subscribe__trust" aria-label="Payment trust points">
          <span>Payfast.io onsite modal</span>
      ...
      <form class="payfast-subscribe__panel" (ngSubmit)="subscribe(selectedPlanCode)">
        ...
        <button class="payfast-subscribe__cta" type="submit" [disabled]="busy">
          {{ busy ? 'Opening Payfast.io...' : ('Subscribe to ' + selectedPlanName) }}
        </button>
...
export class PayfastSubscribeComponent implements OnInit {
  ...
  async subscribe(planCode: string): Promise<void> {
    ...
    this.statusMessage = 'Creating a secure Payfast.io checkout session...';
    ...
    try {
      ...
      if (!response.ok || !session.uuid) {
        throw new Error(session.message || 'Payfast.io is not ready yet.');
      }
      ...
      if (!launcher) {
        throw new Error('Payfast.io checkout could not be loaded.');
      }

      this.statusMessage = 'Payfast.io is open. Complete payment in the secure window.';
      launcher({ uuid: session.uuid }, (result) => {
        this.statusTone = result ? 'success' : 'neutral';
        this.statusMessage = result ? 'Payment complete. Your subscription is being activated.' : 'Payfast.io checkout was closed before completion.';
      });
    } catch (error) {
      this.statusTone = 'error';
      this.statusMessage = error instanceof Error ? error.message : 'Could not start Payfast.io checkout.';
    } finally {
      this.busy = false;
    }
  }
  ...
  private async loadPlans(): Promise<void> {
    ...
    this.statusTone = 'neutral';
    this.statusMessage = 'Payfast.io plans are loading from the API; fallback pricing is shown for now.';
  }
  ...
  private async loadPayfastEngine(src: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if (existing) {
        ...
        existing.addEventListener('error', () => reject(new Error('Payfast.io checkout script failed to load.')), { once: true });
        return;
      }
      ...
      script.onerror = () => reject(new Error('Payfast.io checkout script failed to load.'));
